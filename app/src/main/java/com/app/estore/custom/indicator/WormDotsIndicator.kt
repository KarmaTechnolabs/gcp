package com.cesano.custom.indicator
...
import com.cesano.custom.indicator.BaseDotsIndicator.Type.WORM
...
class WormDotsIndicator @JvmOverloads constructor(
    ...
    init {
        ...
        val horizontalPadding = dpToPx(0)
        setPadding(0, 0, 0, 0)
        clipToPadding = true
    ...
    override fun addDot(index: Int) {
        ...
        dots.add(dot.findViewById<View>(R.id.dot) as ImageView)
    ...
    private fun buildDot(stroke: Boolean): ViewGroup {
        val dot = LayoutInflater.from(context).inflate(
            R.layout.custom_dot_indicator, this,
            false
        ) as ViewGroup
        if (VERSION.SDK_INT >= VERSION_CODES.JELLY_BEAN_MR1) {
            dot.layoutDirection = View.LAYOUT_DIRECTION_LTR
        }
        val dotImageView = dot.findViewById<View>(R.id.dot)
